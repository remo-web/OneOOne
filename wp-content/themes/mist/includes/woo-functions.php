...
if( ! class_exists( 'ZozoWooConfig' ) ) {
    ...
    class ZozoWooConfig {
		...
		function zozo_woo_after_shop_loop_item() {
			echo '<a href="' . get_permalink() . '" class="woo-show-details" title="' . esc_html__( 'Detalhes', 'zozothemes' ) . '">' . esc_html__( 'Show Details', 'zozothemes' ) . '</a></div></div>';
		}
		...
		function zozo_woocommerce_share() {
			...
			if( isset( $zozo_options['zozo_woo_social_sharing'] ) && $zozo_options['zozo_woo_social_sharing'] == 1 ) {
				...
				echo apply_filters( 'zozo_woo_socials_share_title', '<span>' . __( 'Compartilhar:', 'zozothemes' ) . '</span>' );
		...
		<div class="woo-cart-contents">			
			<?php foreach( $woocommerce->cart->cart_contents as $cart_item_key => $cart_item ) { ?>
				<div class="woo-cart-item clearfix">
					...
					<?php echo apply_filters( 'woocommerce_cart_item_remove_link', sprintf('<a href="%s" class="remove remove-cart-item" title="%s" data-cart_id="%s">&times;</a>', esc_url( WC()->cart->get_remove_url( $cart_item_key ) ), esc_html__('Remover este item', 'zozothemes'), $cart_item_key ), $cart_item_key ); ?>
			...
			<div class="woo-cart-links clearfix">
				<div class="cart-link"><a class="btn btn-cart" href="<?php echo get_permalink(get_option('woocommerce_cart_page_id')); ?>" title="<?php esc_html_e('Carrinho', 'zozothemes'); ?>"><?php esc_html_e('Ver Carrinho', 'zozothemes'); ?></a></div>
				<div class="checkout-link"><a class="btn btn-checkout" href="<?php echo get_permalink(get_option('woocommerce_checkout_page_id')); ?>" title="<?php esc_html_e('Finalizar', 'zozothemes'); ?>"><?php esc_html_e('Finalizar Compra', 'zozothemes'); ?></a></div>
			</div>
		</div>
		<?php } ?>
	</div>
...
function zozo_woocommerce_before_cart_table( $args ) {
	...
	if ( $woocommerce->cart->cart_contents_count == 1 ) {
		echo '<h5>' . sprintf( __( 'Seu carrinho contém: %d Item', 'zozothemes' ), $woocommerce->cart->cart_contents_count ) . '</h5>';
	} else {
		echo '<h5>' . sprintf( __( 'Seu carrinho contém: %d Itens', 'zozothemes' ), $woocommerce->cart->cart_contents_count ) . '</h5>';
	}
